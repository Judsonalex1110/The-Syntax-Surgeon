...
Original file is located at
    https://colab.research.google.com/drive/1satTpDimpF3lM96KmNnR1M4FAURyjVkj
"""
...
def detect_python_code(text):
    code_patterns = [r'def\s+\w+\s*\(', r'class\s+\w+\s*', r'import\s+\w+', r'from\s+\w+\s+import', r'if\s+.:', r'for\s+.\sin\s+', r'while\s+.*:']
...
def download_pdf():
    ...
    try:
        ...
        for user, assistant in conversation_history:
            story.append(Paragraph(f"<b>User:</b> {user}", user_style))
            story.append(Paragraph(f"<b>Assistant:</b> {assistant}", assistant_style))
...
story.append(Paragraph(
    f"<b>Exported on:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    styles['Normal']
))
story.append(Paragraph(
    f"<b>Total Messages:</b> {len(conversation_history)}",
    styles['Normal']
))
...
# Make sure your load_model function is defined properly
def load_model():
    # Example: replace with your actual model loading code
    model = "YOUR_MODEL_OBJECT"  # Replace this with your real model initialization
    print("✅ Model loaded successfully!")
    return model

model = load_model()  # Call the function and store the loaded model

with gr.Blocks(title="Granite Chatbot", theme=gr.themes.Soft()) as demo:
    gr.Markdown("""## Granite Chatbot | Chat + Python Code Fixer - No API Keys""")
